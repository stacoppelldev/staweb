...
class HomeView(TemplateView):
    def get_context_data(self, **kwargs):
        ...
        context['sundayMassMalayalam1'] = time.objects.filter(title='Holy Qurbana Malayalam1').first()
        context['sundayMassMalayalam'] = time.objects.filter(title='Holy Qurbana Malayalam').first()
        context['sundayMassEnglish'] = time.objects.filter(title='Holy Qurbana English').first()
        #context['weekdayMassMTWS'] = time.objects.filter(title='Weekday Mass MTWS').first()
        #context['weekdayMassTTH'] = time.objects.filter(title='Weekday Mass TTH').first()
        context['weekdayMassTS'] = time.objects.filter(title='Weekday Mass TS').first()
        context['weekdayMassMF'] = time.objects.filter(title='Weekday Mass MF').first()
        context['weekdayExtMass'] = time.objects.filter(title='Weekday Extension Mass').first()
        #context['adorationMTWS'] = time.objects.filter(title='Adoration MTWS').first()
        #context['adorationTHF'] = time.objects.filter(title='Adoration THF').first()
        context['adorationTS'] = time.objects.filter(title='Adoration TS').first()
        context['confession'] = time.objects.filter(title='Confession').first()
        try:
